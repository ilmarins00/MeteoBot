...
# ====================================================================
# FUNZIONI STORICO E TENDENZA BAROMETRICA
# ====================================================================
...
def genera_grafico_24h(storico):
    ...
    try:
        ...
        fig.suptitle('Stazione Meteo Ecowitt Wittboy — La Spezia (24h)', fontsize=14, fontweight='bold')
...
# ====================================================================
# CLASSIFICAZIONE MASSA D'ARIA (Bergeron 1928, Bolton 1980)
# ====================================================================
...
# ====================================================================
# VALUTAZIONE INSTABILITÀ CONVETTIVA
# ====================================================================
...
# ====================================================================
# OPEN-METEO: FETCH PROFILO VERTICALE
# ====================================================================
...
# ====================================================================
# CALCOLA E SALVA SBCAPE (entry-point standalone --sbcape)
# ====================================================================
...
def fetch_ecowitt_data(max_retries=3):
    ...
    for attempt in range(max_retries):
        ...
        rain_rate = _ecowitt_val(rainfall, "rain_rate")
        rain_hourly = _ecowitt_val(rainfall, "hourly")
        rain_daily = _ecowitt_val(rainfall, "daily")
...
def esegui_report(force_send=False, target_chat_id=None):
    ...
    # ── 1. LETTURA DATI STAZIONE ───────────────────────────────────
    ecowitt = fetch_ecowitt_data()

    if ecowitt is None:
        # Fallback: stazione WMO esterna
    ...
    # Riduzione pressione al livello del mare (formula ipsometrica)
    h = ELEVATION  # altitudine stazione
    Rd = 287.05
    g_val = 9.80665
    ...
    # ── 3. CALCOLO API AVANZATO (Bilancio Idrico Suolo) ────────────
    mese_corrente = now_it.month
    oggi_str = now_it.strftime("%Y-%m-%d")
    giorno_anno = now_it.timetuple().tm_yday

    # EVAPOTRASPIRAZIONE POTENZIALE — Hargreaves-Samani (FAO)
    LAT = LATITUDE
    ...
    # FAO-56 Dual Kc — Kcb (traspirazione basale)
    kcb_mensile = {
    ...
    # Parametri suolo (La Spezia - Foce)
    capacita_campo = 200
    wilting_point = 80
    K_sat = 2.5
    beta_drenaggio = 3.5
    ...
    if os.path.exists(FILE_MEMORIA):
        try:
            with open(FILE_MEMORIA, "r") as f:
                dati_salvati = json.load(f)
            print(f"{debug_msg} | ✓ Letto: {dati_salvati}")
        except Exception as e:
            print(f"{debug_msg} | ✗ Errore: {e}")
            dati_salvati = {}
    else:
        print(f"{debug_msg} | ✗ NON trovato")
    ...
    # Accumulo ETP: media mobile pesata
    ...
    # ── 5. TENDENZA BAROMETRICA + STORICO ──────────────────────────
    ...
    # ── 6. AVVISI (soglie ARPAL Protezione Civile Liguria – Zona C) ─
    avvisi = []
    ...
    if devo_inviare:
        ...
        # Grafico 24h (11:58/59 e 23:58/59)
